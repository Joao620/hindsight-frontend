...
export function MicPopup({ className, writeTranscribedText }: MicPopupProps) {
    ...
    if (recordingState.kind == AudioStates.Error && !recordingState.removeTipTimeout) {
            ...
            case AudioStates.Hearing:
                ...
                recordingState
                    ...
                    .then((audioBlob) => {
                        ...
                        fetch(`${HTTP_PROTOCOL}://${SERVER_URL}/transcribe`, {
                          method: 'POST',
                          body: audioBlob,
                        })
                          .then((response) => {
                              ...
                              setRecordingState({ kind: AudioStates.Idle });
                              response.json().then(response => writeTranscribedText(response.data))
                          })
                          .catch((error) => {
                            console.error(error)
                            setRecordingState({ kind: AudioStates.Error, message: 'Transcription failed. Please try again.' });
                          })
                    })
                    .catch((err) => {
                        const message =
